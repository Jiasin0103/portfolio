...
onMounted(() => {
  ...
  const interactionTarget = containerElement.parentElement ?? containerElement
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uNormalBlend: { value: props.blendMode === 'normal' ? 1 : 0 },
      uTime: { value: 0 },
      uFade: { value: 0 }
    },
    transparent: true,
  ...
  const render = (now: number) => {
    ...
    program.uniforms.uPointCount.value = clamp(Math.round(props.trailLength), 2, MAX_POINTS)
    program.uniforms.uColor.value = hexToRgb(props.color)
    program.uniforms.uSecondaryColor.value = hexToRgb(props.secondaryColor)
    program.uniforms.uTrailWidth.value = Math.max(props.trailWidth, 0.1)
    program.uniforms.uTaper.value = clamp(props.trailTaper, 0, 1)
    program.uniforms.uGlowIntensity.value = Math.max(props.glowIntensity, 0)
    program.uniforms.uGlowSpread.value = Math.max(props.glowSpread, 0)
    program.uniforms.uHotspot.value = clamp(props.hotspot, 0, 1)
    program.uniforms.uBrightness.value = Math.max(props.brightness, 0)
    program.uniforms.uOpacity.value = clamp(props.opacity, 0, 1)
    program.uniforms.uPulseSpeed.value = props.pulseSpeed
    program.uniforms.uNoiseStrength.value = clamp(props.noiseStrength, 0, 1)
    program.uniforms.uNormalBlend.value = props.blendMode === 'normal' ? 1 : 0
...
<template>
  <div ref="container" class="glow-cursor" aria-hidden="true">
    <canvas
      ref="canvas"
      class="glow-cursor__canvas"
